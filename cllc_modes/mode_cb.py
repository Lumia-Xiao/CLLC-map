...
class CBSolver(ModeSolverBase):
    ...
    CB mode
    Given: F, k, P
    Solve: M, theta_c, theta_b, C1C, C2C, C3C, C4C, C1B, C2B, V2B0
    """

    mode_name = "CB"

    def __init__(self) -> None:
        # M guess is set to 1.0 as requested
        self.initial_guess = np.array(
            [1.0, 2.51, 1.00, 0.04, 0.34, -2.37, -0.23, 0.31, -0.01, 0.01],
            dtype=float,
        )
    ...
    def _equations(self, x: np.ndarray, op: OperatingPoint) -> np.ndarray:
        p = self._unpack(x)
        stage_c, stage_b = self._build_stages(p, op)
        ...
        return np.array(
            [
                ...
                p.theta_c + p.theta_b - math.pi / op.F,
    ...
    @staticmethod
    def _make_join(theta_c: float, half_period: float, f_c, f_b):
        def joined(theta: np.ndarray | float):
            ...
            out[m1] = -np.asarray(f_c(t[m1] + half_period), dtype=float)
            out[m2] = -np.asarray(f_b(t[m2] - theta_c + half_period), dtype=float)
            out[m3] = np.asarray(f_c(t[m3]), dtype=float)
            out[m4] = np.asarray(f_b(t[m4] - theta_c), dtype=float)
    ...
    def _build_waveforms(
        self, p: CBParams, op: OperatingPoint
    ...
    def solve(self, op: OperatingPoint) -> SolveResult:
        ...
        checks = {
            "Stage C": check_positive_over_interval(
                lambda th: np.asarray(stage_c["I1"](th), dtype=float)
                - np.asarray(stage_c["Ima"](th), dtype=float),
                0.0,
                p.theta_c,
                tol=config.CHECK_TOL,
                n=config.DENSE_CHECK_POINTS,
            ),
            "Stage B": check_bounded_over_interval(
                stage_b["Vo"],
                -p.M,
                p.M,
                0.0,
